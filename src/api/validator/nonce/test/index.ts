...
async function inputValidationTest() {
  ...
  try {
    await getNonce(undefined as any);
  ...
  try {
    await getNonces('not-an-array' as any);
...
async function edgeCasesTest() {
  ...
  for (let i = 1; i < sameNonces.length; i++) {
    const currentNonce = sameNonces[i];
    const previousNonce = sameNonces[i-1];
    if (currentNonce && previousNonce && currentNonce.lt(previousNonce)) {
