...
async function testInvalidPrivateKeyError() {
  ...
  try {
    await importEd448WalletFromPrivateKey('invalid-base58-key');
  } catch (error) {
    console.log('Expected error caught:', error.message);
  }
  console.log('');
...
async function testInvalidPublicKeyError() {
  ...
  try {
    await importEd448WalletFromPublicKey('invalid-base58-key');
  } catch (error) {
    console.log('Expected error caught:', error.message);
  }
  console.log('');
...
async function testWrongKeyLengthErrors() {
  ...
  try {
    const wrongLengthKey = bs58.encode(Buffer.alloc(32)); // 32 bytes instead of 56
    await importEd448WalletFromPrivateKey(wrongLengthKey);
  ...
  try {
    const wrongLengthPubKey = bs58.encode(Buffer.alloc(32)); // 32 bytes instead of 56
    await importEd448WalletFromPublicKey(wrongLengthPubKey);
...
/**
 * Test 14: Note about ed448 implementation
 */
async function testImplementationNote() {
  console.log('⚠️ Test 14: Implementation Note');
 ...
 */
async function runAllEd448Tests() {
...
export {
  ...
  testImplementationNote
};

// Export the main test function
export default async function test() {
  return runAllEd448Tests();
}
