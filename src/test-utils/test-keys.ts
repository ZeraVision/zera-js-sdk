...
export interface TestKeyData extends TestKeyPair {}
...
export interface TestFeeConfig {
  baseFeeId: string;
  baseFee?: string;
...
export interface OutputSpec {
  person: 'alice' | 'bob' | 'charlie' | 'jesse';
  amount: AmountInput;
  memo?: string;
}
 ...
 */
export function getTestKeyData(keyType: 'ed25519' | 'ed448', person: 'alice' | 'bob' | 'charlie'): TestKeyData {
 ...
 */
export function getTestInput(keyType: 'ed25519' | 'ed448', person: 'alice' | 'bob' | 'charlie'): TestInput {
  const keys = keyType === 'ed448' ? ED448_TEST_KEYS : ED25519_TEST_KEYS;
  const keyPair = keys[person];
 ...
 */
export function createTestInput(keyType: 'ed25519' | 'ed448', person: 'alice' | 'bob' | 'charlie', amount: AmountInput, feePercent: string = '100'): TestInput {
 ...
 */
export function createTestInputs(inputSpecs: InputSpec[]): TestInput[] {
 ...
 */
export function getTestOutput(person: 'alice' | 'bob' | 'charlie' | 'jesse', amount: AmountInput, memo: string = ''): TestOutput {
 ...
 */
export function createTestTransaction(keyType: 'ed25519' | 'ed448', fromPerson: 'alice' | 'bob' | 'charlie', toPerson: 'alice' | 'bob' | 'charlie' | 'jesse', amount: AmountInput, memo: string = '') {
  const input = getTestInput(keyType, fromPerson);
  input.amount = amount;
  return {
    inputs: [input],
 ...
 */
export function createMultiTestTransaction(inputSpecs: InputSpec[], outputSpecs: OutputSpec[]) {
 ...
 */
export const DEFAULT_TEST_FEE_CONFIG: TestFeeConfig = {
  baseFeeId: '$ZRA+0000',
  baseFee: '0.001',
 ...
 */
export const MINIMAL_TEST_FEE_CONFIG: TestFeeConfig = {
  baseFeeId: '$ZRA+0000'
};
